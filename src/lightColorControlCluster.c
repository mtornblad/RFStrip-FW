...
static ZCL_Status_t moveToColorTemperatureInd(ZCL_Addressing_t *addressing, uint8_t payloadLength, ZCL_ZllMoveToColorTemperatureCommand_t *payload)
{
  ...
  {
    miredValue = colorControlClusterServerAttributes.colorTempPhysicalMin.value;
    clippedStatus = true;
  }
  else if (colorControlClusterServerAttributes.colorTempPhysicalMax.value < payload->colorTemperature)
  {
    miredValue = colorControlClusterServerAttributes.colorTempPhysicalMax.value;
    clippedStatus = true;
  }
  else
  {
